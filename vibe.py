...
console = Console()
MODEL = os.getenv("VIBE_MODEL", "distributed")  # Modelo distribuido del cluster
OLLAMA_HOST = os.getenv("OLLAMA_HOST", "http://192.168.1.11:8080")  # URL del cluster

# Configurar cliente de Ollama para usar el cluster
...
class Tools:
    ...
    def list_models() -> ToolResult:
        ...
        try:
            ...
            output_lines = ["Modelos disponibles en el cluster:\n"]
    ...
    while True:
        ...
        if user_input.lower() == '/models':
            try:
                ...
                console.print(f"\n[bold cyan]Modelos disponibles en el cluster ({OLLAMA_HOST}):[/]")
...
if __name__ == "__main__":
    try:
        ...
        console.print(f"[dim]Conectando al cluster: {OLLAMA_HOST}[/]")
        models = ollama_client.list()
        if not models.get('models'):
            console.print("[red]No hay modelos disponibles en el cluster.[/]")
            console.print("[yellow]Verifica que el master esté corriendo y tenga workers conectados[/]")
        else:
            console.print(f"[green]OK - Conectado al cluster ({len(models.get('models', []))} modelos disponibles)[/]\n")
            vibe_chat()
    except Exception as e:
        console.print(f"[red]Error al conectar con el cluster: {str(e)}[/]")
        console.print(f"[yellow]Verifica que el master esté corriendo en: {OLLAMA_HOST}[/]")
        console.print("[yellow]Para usar Ollama local, configura: export OLLAMA_HOST=http://localhost:11434[/]")
